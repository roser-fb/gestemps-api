...
router.get("/", (req, res) => {
  connection.query(
    "SELECT * FROM festius order by data_ini",
    (error, results) => {
      if (error) {
...
router.post("/", (req, res) => {
  ...
  connection.query(
    "INSERT INTO festius (data_ini, motiu, fix) VALUES (?,?,?)",
    [periode.data_ini.toString(), periode.motiu, periode.fix],
    (error) => {
      if (error) {
        console.error(error);
        res.status(500).json({ status: "error" });
...
router.get("/delete/:id", (req, res) => {
  ...
  connection.query("DELETE FROM festius WHERE id = ?", [periodeId], (error) => {
    if (error) {
      console.error(error);
      res.status(500).json({ status: "error" });
    } else {
